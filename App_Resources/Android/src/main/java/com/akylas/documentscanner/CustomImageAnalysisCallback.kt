...
constructor(context: Context, private val cropView: CropView? = null) : ImageAnalysisCallback {
    ...
    companion object {
        ...
        /**
         * @property cropperOffsetWhenCornersNotFound if we can't find document corners, we set corners
         * to image size with a slight margin
         */
        ...
        fun cropDocument(
            ...
            return cropDocument(bitmap, quads)
        }
            ...
            quadsFromJSONString(quads)!!.forEachIndexed { index, quad ->
                ...
                val cropWidth =
                    ((corners.topLeftCorner.distance(corners.topRightCorner) +
                            corners.bottomLeftCorner.distance(corners.bottomRightCorner)) /
                            2)
                        .toInt()
                val cropHeight =
                    ((corners.bottomLeftCorner.distance(corners.topLeftCorner) +
                            corners.bottomRightCorner.distance(corners.topRightCorner)) / 2)
                        .toInt()
    ...
    override fun process(
        ...
        try {
            ...
            var pointsList: List<List<Point>>? = getDocumentCorners(
                previewBitmap,
            ...
            if (pointsList != null) {
                var photoHeight: Int = if (info.rotationDegrees == 180 || info.rotationDegrees == 0) {
                    image.height
                } else {
                    image.width
                }

<script lang="ts">
    ...
    function showEdgesMat(mat, rotation = 0) {
        ...
        console.log('debugEdgesImage', image, debugEdgesImage);
    }

    function showResizedMat(mat, rotation = 0) {
        console.log('showResizedMat', mat);
        if (!mat) {
        ...
        console.log('debugResizedImage', image, debugResizedImage);
    }
    ...
    async function handleFullResult(data: { id: number; nativeDatas?: { [k: string]: any }; [k: string]: any }) {
        ...
        console.log('handleFullResult', data, nativeImages);
        try {
        ...
        <gridLayout row={1} rows="auto,*,auto" padding="10">
            <stackLayout orientation="horizontal" backgroundColor="#33000000" borderRadius="4">
                ...
                <gridLayout rows="auto" columns="auto">
                    ...
                    <label text={computeOptions.algo + ''} fontSize="11" verticalAlignment="bottom" horizontalAlignment="right" paddingRight="10" />
                </gridLayout>
                <gridLayout rows="auto" columns="auto">
                    ...
                    <label text={computeOptions.pageContourType + ''} fontSize="11" verticalAlignment="bottom" horizontalAlignment="right" paddingRight="10" />
                </gridLayout>
                <gridLayout rows="auto" columns="auto">
                    <mdbutton variant="flat" class="icon-btn" text="mdi-feature-search-outline" on:tap={() => setOptionsKey('boundType', (computeOptions.boundType + 1) % 4)} />
                    <label text={computeOptions.boundType + ''} fontSize="11" verticalAlignment="bottom" horizontalAlignment="right" paddingRight="10" />
                </gridLayout>
                <mdbutton
            ...
            {#if computeOptions.debug}
                ...
                <image
                    row={1}
                    stretch="aspectFit"
                    src={showEdges ? debugEdgesImage : debugResizedImage}
                    width="50%"
                    height="30%"
            ...
            <collectionView items={images} verticalAlignment="bottom" horizontalAlignment="right" width="50" rowSpan="3">
                <Template let:item>
                    <gridLayout columns="auto" padding="4">
                        <image src={item} width="100%" elevation="2" borderRadius="4" rippleColor={accentColor} on:tap={() => onImageTap(item)} />
                    </gridLayout>
                </Template>
            </collectionView>
            {#if currentImage}

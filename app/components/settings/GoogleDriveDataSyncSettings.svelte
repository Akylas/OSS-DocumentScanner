<script lang="ts">
    ...
    import OAuthSettingsView from './OAuthSettingsView.svelte';

    $: ({ colorError, colorOnError, colorOnSurfaceVariant, colorOutline, colorPrimary, colorSecondary } = $colors);

    export let data: GoogleDriveDataSyncOptions = null;
    const store = writable(Object.assign({ autoSync: false, enabled: true, color: SERVICES_SYNC_COLOR['gdrive_data'] as string | Color, remoteFolder: 'DocumentScanner' }, data));

    let oauthView: OAuthSettingsView;

    async function save() {
        if (await oauthView?.validateSave()) {
            if (__ANDROID__) {
...
<page id="googledrivesyncsettings" actionBarHidden={true}>
    <gridlayout class="pageContent" rows="auto,*">
        ...
        <CActionBar canGoBack modalWindow={true} title="Google Drive">

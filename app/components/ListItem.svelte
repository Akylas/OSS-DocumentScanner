...
<gridlayout rippleColor={$colors.colorPrimary} on:tap={(event) => dispatch('tap', event)}>
    <canvaslabel padding="16" on:draw={onDraw}>
        ...
        <cgroup paddingLeft={(leftIcon ? 40 : 0) + extraPaddingLeft} textAlignment="left" verticalAlignment="middle">
            ...
            <cspan color={$colors.colorOnSurfaceVariant} fontSize="13" text={subtitle ? '\n' + subtitle : ''} />
        </cgroup>
        <line color={$colors.colorOutline} height="1" startX="0" startY="0" stopX="100%" stopY="0" strokeWidth="1" verticalAlignment="bottom" visibility={showBottomLine ? 'visible' : 'hidden'} />
    </canvaslabel>
</gridlayout>

 ...
 * @ignore
 */
export interface AsPlainObject {
...
export default class MiniSearch<T = any> {
    ...
    static loadJS<T = any>(js: AsPlainObject, options: Options<T>): MiniSearch<T> {
        const { index, documentCount, nextId, documentIds, fieldIds, fieldLength, averageFieldLength, storedFields, dirtCount, serializationVersion } = js;
...
const getOwnProperty = (object: any, property: string) => (Object.prototype.hasOwnProperty.call(object, property) ? object[property] : undefined);
...
const calcBM25Score = (termFreq: number, matchingCount: number, totalCount: number, fieldLength: number, avgFieldLength: number, bm25params: BM25Params): number => {
...
const defaultAutoSuggestOptions = {
    combineWith: AND,
    prefix: (term: string, i: number, terms: string[]): boolean => i === terms.length - 1
};
